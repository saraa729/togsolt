...
function createLedgerService({ db, audit, id, now, money, addMoney }) {
  function addEscrowEntry(orderId, orderItemId, sellerId, type, amount, note) {
    const entry = {
      id: id('esc'),
      orderId,
      orderItemId,
  ...
  function ledgerBalances(sellerId = null) {
  ...
  function reconciliationForDate(dateText) {
  ...
  function syncOrderEscrowStatus(orderId) {
  ...
  function freezeEscrowForOrderItem(item, actorId, reason) {
  ...
  function releaseEscrowForOrderItem(item, actorId) {
  ...
  function refundEscrowForOrderItem(item, actorId, reason = 'Order item cancelled before delivery.') {
    if (!['held', 'disputed'].includes(item.escrowStatus)) return null;
