...
function createCatalogService({ db, httpError, localize, INVENTORY_TYPES }) {
  function sellerShop(sellerId) {
    return db.shops.find((shop) => shop.sellerId === sellerId);
  }

  function tracksPhysicalStock(product) {
    return ['ready_made', 'limited_stock', 'one_of_one'].includes(product.inventoryType);
  }

  function assertPurchasableQuantity(product, quantity) {
  ...
  function shippingInfoForProduct(product, locale = 'mn') {
  ...
  function productResponse(product, locale = 'mn', currency = 'MNT') {
