...
type RouteMetric = { count: number; errors: number; totalDurationMs: number };
type JobMetric = { runs: number; failures: number; lastRunAt: string | null; lastDurationMs: number };

const metrics = {
  httpRequests: 0,
  httpErrors: 0,
  routeCounts: {} as Record<string, RouteMetric>,
  jobs: {} as Record<string, JobMetric>,
  gauges: {} as Record<string, { labels: Record<string, string>; value: number }>,
...
function setGauge(name: string, value: number, labels: Record<string, string> = {}) {
  metrics.gauges[labelKey(name, labels)] = { labels: { __name: name, ...labels }, value };
}
...
function prometheus() {
  ...
  for (const item of Object.values(metrics.gauges)) {
    const { __name, ...labels } = item.labels;
    lines.push(`${__name}${formatLabels(labels)} ${item.value}`);
  }
  return `${lines.join('\n')}\n`;
}

module.exports = { recordRequest, recordJob, setGauge, snapshot, prometheus };
