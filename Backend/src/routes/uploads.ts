...
module.exports = function registerUploads(ctx) {
  const { route, ROLES, db, now, id, httpError, saveState, audit, readRaw, requireRole, scanFile, saveImage, saveThumbnail } = ctx;
  ...
  route('POST', '/uploads/images', async (ctx) => {
    ...
    const scan = await scanFile({ fileName, contentType: parsed.type, data: parsed.data });
