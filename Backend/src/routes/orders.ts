...
module.exports = function registerOrders(ctx) {
  ...
  route('POST', '/checkout', async (ctx) => {
    ...
    const body = await readJson(ctx.req);
    const cart = getOrCreateCart(user.id);
    if (!body.cartId || body.cartId !== cart.id) {
      throw httpError(409, 'cart_required', 'Checkout must be linked to the active cart.');
    }
  ...
  route('POST', '/seller/payout-requests', async (ctx) => {
    ...
    const body = await readJson(ctx.req);
    const currency = assertSupportedCurrency(body.currency || 'MNT');
    const activeRequest = db.payoutRequests.find((item) => item.sellerId === user.id && item.amount?.currency === currency && item.status === 'requested');
    if (activeRequest) throw httpError(409, 'payout_request_pending', 'A payout request is already waiting for admin review.');
    const balances = ledgerBalances(user.id);
    const pendingRequested = db.payoutRequests
      .filter((item) => item.sellerId === user.id && item.amount?.currency === currency && item.status === 'requested')
      .reduce((sum, item) => sum + Number(item.amount?.amount || 0), 0);
    const available = Number(balances.sellerBalance?.[currency] || 0) - pendingRequested;
    ...
    const bankAccount = normalizeBankAccount(body.bankAccount || sellerShop(user.id)?.payout);
    const request = {
      id: id('por'),
      sellerId: user.id,
      status: 'requested',
