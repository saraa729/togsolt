...
const { WEB_ORIGIN, NODE_ENV } = require('../config/constants');
...
module.exports = function registerAuth(ctx) {
  ...
  async function assertLoginAllowed(email) {
    ...
    if (entry.lockedUntil && entry.lockedUntil > Date.now()) {
      throw httpError(429, 'too_many_attempts', 'Олон удаа буруу оролдлоо. Түр хүлээгээд дахин оролдоно уу.');
    }
    if (entry.lockedUntil && entry.lockedUntil <= Date.now()) {
  ...
  async function recordLoginFailure(email) {
    ...
    entry.count += 1;
    if (entry.count >= LOGIN_MAX_ATTEMPTS) entry.lockedUntil = Date.now() + LOGIN_LOCK_MS;
    await runtimeStore.set(`login:${email}`, entry, LOGIN_FAILURE_TTL_MS);
  }
  ...
  const loginUser = async (ctx) => {
    ...
    if (!user || !verifyPassword(body.password, user.passwordHash)) {
      ...
      throw httpError(401, 'bad_credentials', 'Имэйл эсвэл нууц үг буруу байна.');
    }
  ...
  route('POST', '/auth/forgot-password', async (ctx) => {
    ...
    if (user) {
      ...
      const link = `${WEB_ORIGIN}/reset-password?token=${encodeURIComponent(reset.token)}`;
      await mailer.send({
        to: user.email,
