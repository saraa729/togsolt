...
module.exports = function registerAdmin(ctx) {
  ...
  function payoutForRequest(request) {
    return request.payoutId ? db.payouts.find((payout) => payout.id === request.payoutId) : null;
  }

  function reservePayoutBalance(request, payout, actorId) {
    if (request.ledgerEntryId) return db.escrowLedger.find((entry) => entry.id === request.ledgerEntryId) || null;
    const entry = addEscrowEntry(null, null, request.sellerId, 'payout_scheduled', request.amount, `Seller payout request ${request.id} reserved.`);
    request.ledgerEntryId = entry.id;
    if (payout) payout.ledgerEntryId = entry.id;
    audit(actorId, 'reserve_payout_balance', 'payout_request', request.id, { amount: request.amount });
    return entry;
  }

  function manualReceipt(request, body, admin) {
    const transactionRef = assertText(body.transactionRef || body.providerRef || body.bankTransactionId, 'transactionRef');
    return {
      ...
      raw: {
        transactionRef,
        bankAccount: normalizeBankAccount(body.bankAccount || request.bankAccount),
        note: body.note || body.adminNote || '',
        recordedBy: admin.id
      },
      createdAt: now()
    };
  ...
  function slaAlerts() {
    const alerts = [];
    for (const shop of db.shops.filter((item) => item.status === 'pending_verification')) {
      const ageHours = hoursSince(shop.createdAt);
      if (ageHours >= 24) alerts.push({ severity: ageHours >= 48 ? 'critical' : 'warning', queue: 'seller_verification', entityId: shop.id, ageHours, message: 'Seller verification is waiting beyond SLA.' });
    }
    for (const report of db.reports.filter((item) => ['open', 'reviewing'].includes(item.status))) {
      const ageHours = hoursSince(report.updatedAt || report.createdAt);
      if (ageHours >= 12) alerts.push({ severity: ageHours >= 24 ? 'critical' : 'warning', queue: 'moderation', entityId: report.id, ageHours, message: 'Moderation report is waiting beyond SLA.' });
    }
    for (const dispute of db.disputes.filter((item) => ['open', 'frozen'].includes(item.status))) {
      const ageHours = hoursSince(dispute.updatedAt || dispute.createdAt);
      if (ageHours >= 24) alerts.push({ severity: ageHours >= 72 ? 'critical' : 'warning', queue: 'dispute', entityId: dispute.id, ageHours, message: 'Dispute resolution is waiting beyond SLA.' });
    }
    for (const request of db.payoutRequests.filter((item) => ['requested', 'approved', 'pending_manual'].includes(item.status))) {
      const ageHours = hoursSince(request.updatedAt || request.createdAt);
      if (ageHours >= 24) alerts.push({ severity: ageHours >= 72 ? 'critical' : 'warning', queue: 'payout', entityId: request.id, ageHours, message: 'Payout request is waiting beyond SLA.' });
    }
  ...
  route('PATCH', '/admin/payout-requests/:requestId', async (ctx) => {
    ...
    if (['paid', 'rejected'].includes(request.status)) throw httpError(409, 'payout_request_closed', 'Payout request is already closed.');
    request.adminNote = body.note || request.adminNote || '';
    request.reviewedBy = admin.id;
    request.updatedAt = now();
    if (body.status === 'rejected') {
      if (request.ledgerEntryId || request.payoutId) throw httpError(409, 'payout_already_reserved', 'Reserved payout requests cannot be rejected.');
      request.status = 'rejected';
      request.rejectedAt = now();
    }
    if (body.status === 'approved') {
      if (request.payoutId) throw httpError(409, 'payout_already_approved', 'Payout request was already approved.');
      const bankAccount = normalizeBankAccount(body.bankAccount || request.bankAccount);
      request.bankAccount = bankAccount || request.bankAccount || null;
      const receipt = await executeBankTransfer(request);
      const payout = {
        id: id('pay'),
        sellerId: request.sellerId,
        status: receipt.status === 'pending_manual' ? 'pending_manual' : (receipt.status === 'submitted' ? 'processing' : receipt.status),
      ...
      request.status = payout.status === 'pending_manual' ? 'pending_manual' : 'approved';
      reservePayoutBalance(request, payout, admin.id);
      request.approvedAt = now();
    }
    if (body.status === 'paid') {
      const receipt = manualReceipt(request, body, admin);
      let payout = payoutForRequest(request);
      if (!payout) {
        payout = {
          id: id('pay'),
          sellerId: request.sellerId,
  ...
  route('GET', '/admin/queues', async (ctx) => {
    ...
    const queues = {
      ...
      payoutQueue: db.payouts.filter((payout) => ['scheduled', 'processing', 'pending_manual'].includes(payout.status)),
      payoutRequests: db.payoutRequests.filter((request) => ['requested', 'approved', 'pending_manual'].includes(request.status))
    };
    return {
      ...queues,
  ...
  route('GET', '/admin/ops/alerts', async (ctx) => {
    requireRole(ctx, ROLES.ADMIN);
  ...
  route('GET', '/admin/audit-logs/export', async (ctx) => {
    requireRole(ctx, ROLES.ADMIN);
    const entityType = ctx.url.searchParams.get('entityType');
    const actorId = ctx.url.searchParams.get('actorId');
    const logs = db.auditLogs.filter((log) => {
    ...
    const rows = [
      ['id', 'createdAt', 'actorId', 'action', 'entityType', 'entityId', 'metadata'],
      ...logs.map((log) => [log.id, log.createdAt, log.actorId, log.action, log.entityType, log.entityId, log.metadata])
    ];
    ctx.sendText(ctx.res, 200, rows.map((row) => row.map(csvCell).join(',')).join('\n'), 'text/csv; charset=utf-8');
    return undefined;
  });
  ...
  route('GET', '/admin/reports/overview', async (ctx) => {
    ...
    return {
      ...
      slaAlerts: slaAlerts().slice(0, 10)
    };
  });
};
