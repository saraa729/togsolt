...
const SCHEMA_FILE = firstExistingPath([
  path.join(__dirname, '..', 'db', 'postgres', 'relational.sql'),
  path.join(__dirname, '..', '..', 'db', 'postgres', 'relational.sql')
]);

async function loadState(pool: any) {
  const source = String(process.env.EXPOCRAFT_RELATIONAL_SOURCE || '').toLowerCase();
  if (source === 'postgres' || source === 'app_state' || (source !== 'json' && !fs.existsSync(DATA_FILE))) {
    const result = await pool.query(`SELECT data FROM app_state WHERE id = 'expocraft'`);
    if (!result.rows[0]) throw new Error('Postgres app_state row not found.');
