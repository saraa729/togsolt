...
test('buyer payment is held in escrow, released after delivery, then paid out', async () => {
  ...
  try {
    ...
    const unlinkedCheckout = await request(baseUrl, 'POST', '/checkout', {
      currency: 'MNT',
      shippingAddress: { country: 'MN', city: 'Ulaanbaatar', line1: 'Demo address' }
    }, login.body.token);
    assert.equal(unlinkedCheckout.status, 409);
    assert.equal(unlinkedCheckout.body.error.code, 'cart_required');
...
test('disputes freeze escrow and admin can refund with ledger reconciliation', async () => {
  ...
  try {
    ...
    const cart = await request(baseUrl, 'POST', '/cart/items', { productId, quantity: 1 }, buyerLogin.body.token);
    const checkout = await request(baseUrl, 'POST', '/checkout', {
      cartId: cart.body.cart.id,
      currency: 'MNT',
      shippingAddress: { country: 'MN', city: 'Ulaanbaatar', line1: 'Demo address' }
...
test('chat reviews reports moderation and admin dashboard are available', async () => {
  ...
  try {
    ...
    const cart = await request(baseUrl, 'POST', '/cart/items', { productId: product.id, quantity: 1 }, buyerLogin.body.token);
    const checkout = await request(baseUrl, 'POST', '/checkout', {
      cartId: cart.body.cart.id,
      currency: 'MNT',
      shippingAddress: { country: 'MN', city: 'Ulaanbaatar', line1: 'Demo address' }
...
test('cart groups multiple sellers and seller can add made-to-order progress media', async () => {
  ...
  try {
    ...
    const checkout = await request(baseUrl, 'POST', '/checkout', {
      cartId: secondCart.body.cart.id,
      currency: 'MNT',
      shippingAddress: { country: 'MN', city: 'Ulaanbaatar', line1: 'Demo address' }
    }, buyerLogin.body.token);
    assert.equal(checkout.status, 200);
...
test('public shop page exposes SEO, artisan profile, products, and translation suggestions', async () => {
  ...
  try {
    ...
    assert.equal(shop.status, 200);
    assert.equal(shop.body.shop.seo.title, 'Нүүдэл Эсгий Урлан | ExpoCraft');
...
test('FR-2 through FR-8 MVP acceptance flow is covered end to end', async () => {
  ...
  try {
    ...
    const checkout = await request(baseUrl, 'POST', '/checkout', {
      cartId: customCart.body.cart.id,
      currency: 'USD',
    ...
    const uniqueCheckout = await request(baseUrl, 'POST', '/checkout', {
      cartId: uniqueCart.body.cart.id,
      currency: 'MNT',
      paymentMethod: 'qpay',
      shippingAddress: { country: 'MN', city: 'Ulaanbaatar', line1: 'Demo address' }
    }, buyerLogin.body.token);
...
test('phase two APIs cover coupons contracts logistics AI and idempotent payment callbacks', async () => {
  ...
  try {
    ...
    const cart = await request(baseUrl, 'POST', '/cart/items', { productId: customProduct.id, quantity: 1 }, buyerLogin.body.token);
    const checkout = await request(baseUrl, 'POST', '/checkout', {
      cartId: cart.body.cart.id,
      currency: 'MNT',
      shippingAddress: { country: 'MN', city: 'Ulaanbaatar', line1: 'Demo address' }
...
test('seller income is isolated and payout request uses released balance', async () => {
  ...
  try {
    ...
    await request(baseUrl, 'PATCH', `/seller/order-items/${itemId}/status`, { status: 'delivered' }, sellerLogin.body.token);
    await request(baseUrl, 'POST', `/orders/${checkout.body.order.id}/confirm-received`, {}, buyerLogin.body.token);
    const beforePayoutBalance = await request(baseUrl, 'GET', '/seller/balance', undefined, sellerLogin.body.token);
    const beforeSellerPayouts = Number(beforePayoutBalance.body.balances.sellerPayouts.MNT || 0);
    const beforeSellerBalance = Number(beforePayoutBalance.body.balances.sellerBalance.MNT || 0);
    ...
    const duplicateRequest = await request(baseUrl, 'POST', '/seller/payout-requests', { currency: 'MNT', amount: 1 }, sellerLogin.body.token);
    assert.equal(duplicateRequest.status, 409);

    const approve = await request(baseUrl, 'PATCH', `/admin/payout-requests/${payoutRequest.body.payoutRequest.id}`, { status: 'approved' }, adminLogin.body.token);
    assert.equal(approve.status, 200);
    assert.equal(approve.body.payoutRequest.status, 'pending_manual');
    assert.equal(approve.body.payoutRequest.receipt.provider, 'manual_bank');

    const paid = await request(baseUrl, 'PATCH', `/admin/payout-requests/${payoutRequest.body.payoutRequest.id}`, {
      status: 'paid',
      transactionRef: 'MN-BANK-0001',
      note: 'Transferred from admin internet bank'
    }, adminLogin.body.token);
    assert.equal(paid.status, 200);
    assert.equal(paid.body.payoutRequest.status, 'paid');
    assert.equal(paid.body.payoutRequest.receipt.providerRef, 'MN-BANK-0001');

    const balances = await request(baseUrl, 'GET', '/seller/balance', undefined, sellerLogin.body.token);
    assert.equal(balances.body.balances.sellerPayouts.MNT, beforeSellerPayouts + 39600);
    assert.equal(balances.body.balances.sellerBalance.MNT, beforeSellerBalance - 39600);
  } finally {
    await new Promise((resolve) => server.close(resolve));
  }
});
